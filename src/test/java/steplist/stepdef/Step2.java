...
public class Step2 {
    ...
    List<String> PartnerId = new ArrayList<>();
    List<String> StatusFullTransaction= new ArrayList<>();
...
//			secretKey = getSecretKey(rowData, partnerId);
            ...
            try {
                ...
                handleResponseXML(response, rowData, "rc");
            } catch (Exception e) {
                // Handle exceptions gracefully
                e.printStackTrace();
                handleFailure();
    ...
    public void i_perform_post_for_inq_pay_rev_XML() throws IOException {
        ...
        for (Map<String, String> rowData : excelData) {
            ...
            StringBuilder concatenatedstring = new StringBuilder();
            // Convert dari jsonArray ke Object json
            JSONObject jsonObject = new JSONObject();
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONArray keyValueArray = jsonArray.getJSONArray(i);
                String key = keyValueArray.getString(0);
            ...
            if (isInq.equalsIgnoreCase("TRUE")) {
                ...
                try {
                    Response response = executeXMLPostRequest(url, convertJsonToXml(jsonObject));
                    handleResponseXML(response, rowData,"Inq");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
                ...
                try {
                    Response response = executeXMLPostRequest(url, convertJsonToXml(jsonObject));
                    handleResponseXML(response, rowData, "Pay");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
                ...
                try {
                    Response response = executeXMLPostRequest(url, convertJsonToXml(jsonObject));
                    handleResponseXML(response, rowData, "Rev");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
                ...
                try {
                    Response response = executeXMLPostRequest(url, convertJsonToXml(jsonObject));
                    handleResponseXML(response, rowData, "Adv");
                } catch (Exception e) {
                    e.printStackTrace();
                    handleFailure();
                }
    ...
    public void i_perform_post_for_inq_pay_rev_XML_NEW() throws IOException {
        // Loop through excelData and make API requests
        String TC = null;
        String trackingref = null;
        for (Map<String, String> rowData : excelData) {
            ...
            if (isInq.equalsIgnoreCase("TRUE")) {
                // Start ***********************************
                PartnerId.add(rowData.get("partnerid") != null ? rowData.get("partnerid") : rowData.get("AgentID"));
                JSONArray jsonArray = buildRequestBody(rowData, trackingref);
                StringBuilder concatenatedstring = new StringBuilder();
                // Convert dari jsonArray ke Object json
                JSONObject jsonObject = new JSONObject();
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONArray keyValueArray = jsonArray.getJSONArray(i);
                    //delete totalamount
                    if(keyValueArray.getString(0).equalsIgnoreCase("totalamount") ) continue;
                    String key = keyValueArray.getString(0);
                    String value = keyValueArray.getString(1);
                    jsonObject.put(key, value);
                    concatenatedstring.append(value);
                ...
                try {
                    Response response = executeXMLPostRequest("inq", convertJsonToXml(jsonObject));
                    handleResponseXMLwithType(response, rowData, "Inq");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
            ...
            if (isPay.equalsIgnoreCase("TRUE")) {
                ...
                try {
                    Response response = executeXMLPostRequest("pay", convertJsonToXml(jsonObject));
                    handleResponseXMLwithType(response, rowData, "Pay");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
            ...
            if (isRev.equalsIgnoreCase("TRUE")) {
                ...
                try {
                    Response response = executeXMLPostRequest("rev", convertJsonToXml(jsonObject));
                    handleResponseXMLwithType(response, rowData, "Rev");
                } catch (Exception e) {
                    // Handle exceptions gracefully
                    e.printStackTrace();
                    handleFailure();
            ...
            if (isAdv.equalsIgnoreCase("TRUE")) {
                // Start ***********************************
                PartnerId.add(rowData.get("partnerid") != null ? rowData.get("partnerid") : rowData.get("AgentID"));
                JSONArray jsonArray = buildRequestBody(rowData, trackingref);
                StringBuilder concatenatedstring = new StringBuilder();
                ...
                try {
                    Response response = executeXMLPostRequest("adv", convertJsonToXml(jsonObject));
                    handleResponseXMLwithType(response, rowData, "Adv");
                } catch (Exception e) {
                    e.printStackTrace();
                    handleFailure();
                }
    ...
    private JSONArray buildRequestBody(Map<String, String> rowData, String trackingref) {
        ...
        for (Map.Entry<String, String> entry : rowData.entrySet()) {
            ...
            Set<String> body = new HashSet<>(Arrays.asList("TestCase", "secretKey", "TableName", "endpoint", "url", "rc", "schema", "type", "sig", "xExternal", "xTimestamp", "xSignature", "xAuth", "xPartner", "isInq", "isAdv", "isPay", "isRev",
                    "rcInq", "rcPay", "rcAdv", "rcRev", "urlInq", "urlPay", "urlAdv", "urlRev"));
            if (!body.contains(key)) {
                // Create a JSONArray for [key, value] pair
                JSONArray innerArray = new JSONArray();
                switch (value) {
...
//    private JSONArray buildRequestBody(Map<String, String> rowData, String trackingref) {
//        Set<String> NonAlfaApiProject = new HashSet<>(Arrays.asList("trxid", "trxdate", "partnerid", "productid", "customerid", "trackingref", "terminalid"));
////        Set<String> NonAlfatotAmtApiProject = new HashSet<>(Arrays.asList("trxid", "trxdate", "partnerid", "productid", "customerid", "totalamount", "trackingref", "terminalid"));
//
//        Set<String> AlfaApiProject = new HashSet<>(Arrays.asList("AgentID", "AgentPIN", "AgentTrxID", "AgentStoreID", "ProductID", "CustomerID", "DateTimeRequest"));
//
//        String trxType = rowData.get("APIProjType");
//        Set<String> selectedKeySet;
//        // Select the key set based on object type
//        switch (trxType) {
//            case "ALFA":
//                selectedKeySet = AlfaApiProject;
//                if ()
//                break;
//            case "NONALFA":
//                break;
//        }
//        if ("ALFA".equalsIgnoreCase(trxType)) {
//
//        } else if ("NONALFA".equalsIgnoreCase(trxType)) {
//            selectedKeySet = NonAlfaApiProject;
//        } else {
...
//            String value = entry.getValue();
//
//            Set<String> body = new HashSet<>(Arrays.asList("TestCase", "url", "rcInq", "rcPay", "rcAdv", "sig", "xExternal", "xTimestamp", "xSignature", "xAuth", "xPartner", "isInq", "isAdv", "isPay", "isRev"));
//            if (!body.contains(key)) {
//                // Create a JSONArray for [key, value] pair
//                JSONArray innerArray = new JSONArray();
//
...
//					totalamount = Integer.toString(intValue);
    ...
    private void handleResponseXML(Response response, Map<String, String> rowData, String trxType) {
        try {
            String ExpectedRc = rowData.get("rc"+trxType);;
...
//				}
    ...
    private void handleResponseXMLwithType(Response response, Map<String, String> rowData, String trxType) {
        try {
            String ExpectedRc = rowData.get("rc"+ trxType);
            String responseBody = response.getBody().asString();
            String rc = extractXmlValueFromResponse(responseBody, "rc");
...
//				}
    ...
    private void generatereport() throws IOException {
        ...
        try {
            ...
            String[] headers = {"Test Case", "Services", "Method", "Uri", "Type", "Request", "Response", "Expected RC", "RC result", "Desc", "Status", "Time taken"};
            ...
            for (String rowData : TestCase) {
                ...
                for (int i = 0; i < headers.length; i++) {
                    ...
                    if (i == 0) cell.setCellValue(TestCase.get(j).split(" ")[0]);
                    if (i == 1) cell.setCellValue(TestCase.get(j).split(" ")[1]);
                    if (i == 2) cell.setCellValue(RequestMethodList.get(j));
                    if (i == 3) cell.setCellValue(RequestUriList.get(j));
                    if (i == 4) cell.setCellValue(ContentTypeList.get(j));
                    if (i == 5) cell.setCellValue(RequestBodyList.get(j));
                    if (i == 6) cell.setCellValue(ResponseBodyList.get(j));
                    if (i == 7) cell.setCellValue(ExpectedRcList.get(j));
                    if (i == 8) cell.setCellValue(Rc.get(j));
                    if (i == 9) cell.setCellValue(Desc.get(j));
                    if (i == 10) cell.setCellValue(Result.get(j));
                    if (i == 11) cell.setCellValue(ResTimeList.get(j));
    ...
    private void generateReportSummary() throws IOException {
        HashMap<String,Integer> partnerIdCountMap = new HashMap();
        HashMap<String,Integer> resTimeMap = new HashMap();
        HashMap<String,Integer> countSuccessMap = new HashMap<>();
        HashMap<String,Integer> countFailedMap = new HashMap<>();
        ...
        try {
            ...
            // Processing report
            int tcTrack = 0;
            for (String pid : PartnerId ) {
                String partnerId = pid;

                // PartnerId TC Total
                int partneridCount = partnerIdCountMap.getOrDefault(partnerId, 0);
                partneridCount++;
                partnerIdCountMap.put(partnerId, partneridCount);

                // ResTime
                int TCresTime = Integer.parseInt(ResTimeList.get(tcTrack));
                int currResTime = resTimeMap.getOrDefault(partnerId, 0);
                TCresTime += currResTime;
                resTimeMap.put(partnerId, TCresTime);

                // TC Success
                int TCSuccess = Result.get(tcTrack).equalsIgnoreCase("PASSED") ? 1 : 0 ;
                int currTCSuccess = countSuccessMap.getOrDefault(partnerId, 0);
                currTCSuccess += TCSuccess;
                countSuccessMap.put(partnerId, currTCSuccess);

                // TC Failed
                int TCFailed = Result.get(tcTrack).equalsIgnoreCase("FAILED") ? 1 : 0 ;
                int currTCFailed = countFailedMap.getOrDefault(partnerId, 0);
                currTCFailed += TCFailed;
                countFailedMap.put(partnerId, currTCFailed);
            ...
            // Create header row
            Row headerRow = sheet.createRow(0);
            String[] headers = {"Test Case Partner Id", "Total TC","Total Step",  "Total Time", "Success Step", "Failed Step"};
            CellStyle headerCellStyle = workbook.createCellStyle();
            headerCellStyle.setFillForegroundColor(IndexedColors.GREY_25_PERCENT.getIndex());
            headerCellStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
            Font headerFont = workbook.createFont();
            ...
            // Get all partnerid
            Set<String> keys = partnerIdCountMap.keySet();
             ArrayList<String> partnerArrayList = new ArrayList<>();

            for (String key : keys) {
                partnerArrayList.add(key);
            }

            // Create data rows
            int rowNum = 1;
            int j = 0; // used for row tracker
            while (j < partnerArrayList.size()) {
                String currPartner = partnerArrayList.get(j);
                Row row = sheet.createRow(rowNum++);
                // scenario.log(headers.length);
                for (int i = 0; i < headers.length; i++) {
                    Cell cell = row.createCell(i);
                    CellStyle cellStyle = workbook.createCellStyle();
                    if (i == 0) cell.setCellValue(currPartner);
                    if (i == 1) cell.setCellValue(excelData.size());
                    if (i == 2) cell.setCellValue(partnerIdCountMap.get(currPartner).toString());
                    if (i == 3)  cell.setCellValue(resTimeMap.get(currPartner).toString());
                    if (i == 4) cell.setCellValue(countSuccessMap.get(currPartner).toString());
                    if (i == 5) cell.setCellValue(countFailedMap.get(currPartner).toString());
