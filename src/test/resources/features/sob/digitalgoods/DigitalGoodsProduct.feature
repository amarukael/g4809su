...
Feature: Digital Goods Product
  ...
  @dgms_product_filter
  Scenario Outline: [SOB][DGMS][FE] Product Master - Product Using Filter by Field -- "<condition>"
    ...
    Examples:
      ...
      | PPOB Name         | VOUCHER TOPUP IN GAME( UNIPIN )            | Success   |
      | Product Name      | PBB Kab. Brebes                            | Success   |
      | PPOB Name         | VOUCHER                                    | Success   |
      | Product Name      | PBB                                        | Success   |
      | PPOB Name         | randomcase,VOUCHER TOPUP IN GAME( UNIPIN ) | Success   |
      | Product Name      | randomcase,PBB Kab. Brebes                 | Success   |
      | Status            | Inactive                                   | Success   |
      | Supplier Name     | JPA                                        | Success   |
      | Category Name     | MULTIFINANCE                               | Success   |
      | Product ID        | PDAMLAMPUNGKOT                             | Success   |
      | Category & Sub    | VOUCHER GAMES,Steam                        | Success   |
      | Product ID        | TEST                                       | Failed    |
      | PPOB Name         | IN GAME                                    | Failed    |
      | Product Name      | Brebes                                     | Failed    |
      | Product ID        | randomcase,PDAMLAMPUNGKOT                  | Failed    |
      | Sub Category Name | Steam                                      | Failed    |
      | PPOB Name         | random,100                                 | Failed    |
      | Product ID        | random,100                                 | Failed    |
      | Product Name      | random,100                                 | Failed    |
      | PPOB Name         | random,200                                 | Failed    |
      | Product ID        | random,101                                 | Failed    |
      | Product Name      | random,101                                 | Failed    |

  @dgms_product_filter_all_field
  Scenario Outline: [SOB][DGMS][FE] Product Master - Product Using Filter by All Field Failed
    Given I am in Menu "Digital Goods" and Sub-Menu "Product Master"
    Then I click Tab Product on DGMS Product Master
    When I click filter button on "Digital Goods Product"
  ...
  Scenario Outline: [SOB][DGMS][FE] Product Master - Product Add New Single Product
    ...
    * I click field "Status" and fill with "<status>" on DGMS Product Master Product
    ...
    Given I click filter button on "Digital Goods Product"
    When I click filter button on "Digital Goods Product"
    * I click field "Product ID" and fill with "<productId>" on DGMS Product Master Product
    When I click apply button filter
    Then Datatable show data "Digital Goods Product"
    ...
    Examples:
      | ppobName | productId          | keterangan | catName       | subCatName  | supplierName | status   |
  ...
  Scenario Outline: [SOB][DGMS][FE] Product Master - Switch Product Status
    ...
    Then I click field "Product Name" and fill with "only" on DGMS Product Master Category
    When I click apply button filter
    Then Datatable show data "Digital Goods Product"
